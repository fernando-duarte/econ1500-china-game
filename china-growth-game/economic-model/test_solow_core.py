...
class TestSolowCore(unittest.TestCase):
    ...
    def test_solve_solow_model(self):
        ...
        self.assertIsInstance(results, np.ndarray, "Results should be a numpy array")
        ...
        for year_idx in range(len(years)):
            ...
            if year_idx > 0:
                self.assertGreater(results[year_idx]['GDP'], 0, f"GDP should be positive in year {years[year_idx]}")
            
            # Capital should be positive
            self.assertGreater(results[year_idx]['Capital'], 0, 
                              f"Capital should be positive in year {years[year_idx]}")
            
            # Labor force should be positive and increasing
            if year_idx > 0:
                self.assertGreater(results[year_idx]['Labor Force'], results[year_idx-1]['Labor Force'], 
                                  f"Labor force should increase from {years[year_idx-1]} to {years[year_idx]}")
            
            # Human capital should be positive and increasing
            if year_idx > 0:
                self.assertGreater(results[year_idx]['Human Capital'], results[year_idx-1]['Human Capital'], 
                                  f"Human capital should increase from {years[year_idx-1]} to {years[year_idx]}")
            
            # TFP should be positive and increasing
            if year_idx > 0:
                self.assertGreater(results[year_idx]['Productivity (TFP)'], results[year_idx-1]['Productivity (TFP)'], 
                                  f"TFP should increase from {years[year_idx-1]} to {years[year_idx]}")
